    ...
    for entry in glob(&format!("{}/*/", cofg.path.tmp_path)).expect(&t!("filesystem.glob_failed")) {
        match entry {
            ...
            Ok(path) => {
                ...
                for entry in glob(&format!("{}/**/*.ts", path.display()))
                    .expect(&t!("filesystem.glob_failed"))
                {
                    match entry {
                        Err(_) => (),
                        Ok(ts_path) => {
...
fn process_boot_json_files(cofg: &Cofg) {
    ...
    for entry in
        glob(&format!("{}/*/boot.json", cofg.path.tmp_path)).expect(&t!("filesystem.glob_failed"))
    {
        match entry {
            Ok(path) => {
...
fn create_mod_zip(
    ...
    // Remove unlisted files and empty directories first
